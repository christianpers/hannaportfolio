...
attribute vec3 aVertexExtraCoord;
...
void main(void) {
  ...
  // vec3 texturePos = texture2D(uTexture, aTextureCoord).rgb;

  // vec3 realPos = texturePos * vec3(2.0) - vec3(1.0);

  vec3 newPos = aVertexPosition;
  // newPos.xy *= vec2(uCounter);

  // float counter = uCounter;
  float counter = sin(uCounter);

  // newPos.xy *= aTextureCoord * vec2(counter * 3.0);
  // newPos.xy *= vec2(counter * 2.0) + aTextureCoord;

	float n = snoise(vec3(newPos.xy/vec2(.9), sin(counter)));
  // float n = snoise(vec3(aVertexPosition.xy, sin(counter/10.0)));


	
	// newPos += vec3(n);

  // float n = snoise(aVertexColor);

  // aVertexColor += 

	
  // newPos.x += .5;
	// float nTwo = snoise(newPos * vec3(counter));
  // float nTwo = snoise(newPos * vec3(counter/5.0));
	// newPos += vec3(nTwo);

	// float nThree = snoise(newPos / vec3(counter));
	// newPos *= nThree;

	// float nFour = snoise(newPos / vec3(counter));
